...
import {
  ...
  Utensils, Ticket, ShoppingBag, MoreHorizontal, Map as MapIcon
} from "lucide-react";
...
function TripDetail({ tripId, lang, displayName, onBack, onToast }) {
  ...
  const load = useCallback(async () => {
    const [{ data: tripRow }, { data: legRows }, { data: memberRows }, { data: te }, { data: fe }, { data: ac }, { data: doc }, { data: ex }, { data: st }] = await Promise.all([
    ...
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "26px 24px 110px" }}>
      ...
      {!section ? (
        ...
        <div style={{ background: COLORS.paper, borderRadius: 18, padding: 26 }}>
          ...
          {section === "planificador" && <PlanificadorTab tripId={tripId} lang={lang} displayName={displayName} trip={trip} legs={legs} stops={stops} onReload={load} />}
...
function TransporteTab({ tripId, legs, displayName, lang, transportEntries, fuelEntries, members, onReload }) {
  const legIdToType = {};
  legs.forEach((l) => { legIdToType[l.id] = l.transport_id; });
  const types = [];
  legs.forEach((l) => { if (!types.includes(l.transport_id)) types.push(l.transport_id); });

  return (
    <div>
      {types.map((typeId, idx) => {
        const opt = TRANSPORT_OPTIONS.find((o) => o.id === typeId) || TRANSPORT_OPTIONS[0];
        const legForType = legs.find((l) => l.transport_id === typeId);
        const typeTransportEntries = transportEntries.filter((e) => legIdToType[e.leg_id] === typeId);
        const typeFuelEntries = fuelEntries.filter((e) => legIdToType[e.leg_id] === typeId);
        return (
          <div key={typeId}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <opt.icon size={17} color={COLORS.rust} />
              <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 15, color: COLORS.ink }}>{t(lang, opt.labelKey)}</span>
            </div>
            <TransportTypeBlock tripId={tripId} legId={legForType.id} opt={opt} displayName={displayName} lang={lang} transportEntries={typeTransportEntries} fuelEntries={typeFuelEntries} members={members} onReload={onReload} />
            {idx < types.length - 1 && <TicketDivider />}
          </div>
        );
      })}
    </div>
...
function TransportTypeBlock({ tripId, legId, opt, displayName, lang, transportEntries, fuelEntries, members, onReload }) {
  ...
  if (opt.category === "none") {
    return (
      <p style={{ fontFamily: "'Space Grotesk', sans-serif", color: COLORS.inkLight, fontSize: 13.5, marginBottom: 20 }}>{t(lang, "none_transport_text", { transport: t(lang, opt.labelKey).toLowerCase() })}</p>
    );
  }
...
function PlanificadorTab({ tripId, lang, displayName, trip, legs, stops, onReload }) {
  const totalDays = trip?.days || (legs.reduce((s, l) => s + Number(l.days || 0), 0)) || 1;
  ...
  const addStop = async () => {
    ...
    await supabase.from("planner_stops").insert({ trip_id: tripId, day: activeDay, title: title.trim(), lat: pending.lat, lng: pending.lng, added_by: displayName });
    setPending(null); setTitle(""); onReload();
  };
    ...
    <div>
      <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 16, paddingBottom: 4 }}>
        {Array.from({ length: totalDays }, (_, i) => i + 1).map((d) => (
          <button key={d} onClick={() => { setActiveDay(d); setPending(null); }} style={{ padding: "7px 14px", borderRadius: 999, whiteSpace: "nowrap", border: `1.5px solid ${activeDay === d ? COLORS.ocean : COLORS.inkLight + "55"}`, background: activeDay === d ? COLORS.ocean : "transparent", color: activeDay === d ? COLORS.paper : COLORS.ink, fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, cursor: "pointer" }}>
            {t(lang, "day_label")} {d}
          </button>
        ))}
      </div>

      <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5, color: COLORS.inkLight, marginBottom: 10 }}>{t(lang, "click_map_hint")}</p>

      <div style={{ borderRadius: 14, overflow: "hidden", marginBottom: 14, border: `1.5px solid ${COLORS.inkLight}33` }}>
        <MapContainer center={center} zoom={12} style={{ height: 340, width: "100%" }} key={center.join(",")}>
          <TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
          <MapClickCatcher onPick={(latlng) => setPending(latlng)} />
          {dayStops.map((s) => (
            <Marker key={s.id} position={[s.lat, s.lng]}><Popup>{s.title}</Popup></Marker>
          ))}
      ...
      {pending && (
        <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
          <input style={{ ...inputStyle, flex: 1 }} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t(lang, "stop_title_placeholder")} autoFocus />
          <Btn onClick={addStop}><Plus size={15} /> {t(lang, "save")}</Btn>
          <Btn variant="ghost" onClick={() => { setPending(null); setTitle(""); }}>{t(lang, "cancel")}</Btn>
        </div>
      )}
