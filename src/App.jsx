...
import {
  ...
  Utensils, Ticket, ShoppingBag, MoreHorizontal, Map as MapIcon, Search, Footprints, Navigation
} from "lucide-react";

L.Marker.prototype.options.icon = L.icon({ iconUrl: markerIcon, shadowUrl: markerShadow, iconAnchor: [12, 41], popupAnchor: [1, -34] });
...
class SectionErrorBoundary extends Component {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{ padding: 20, fontFamily: "'Space Grotesk', sans-serif", color: "#B24A2E", background: "#EFE6CE", borderRadius: 12 }}>
          <strong>Error en esta sección:</strong> {String(this.state.error.message || this.state.error)}
        </div>
      );
    }
...
const TRANSLATIONS = {
  es: {
    ...
    nav_planner: "Planificador", day_label: "Día", stop_title_placeholder: "¿Qué vais a hacer aquí?",
    click_map_hint: "Toca el mapa para añadir una parada al día seleccionado.", no_stops_yet: "Ningún punto marcado para este día todavía.",
    save: "Guardar",
    field_transport_mode: "Medio de transporte", field_from: "Desde", field_to: "Hasta",
    field_departure_date: "Fecha de salida", field_arrival_date: "Fecha de llegada",
    field_participants: "Participantes", field_files_links: "Archivos/links",
    add_transport_entry: "Añadir transporte", no_transport_yet: "Ningún transporte añadido todavía.",
    search_place_placeholder: "Busca un sitio (ej: Torre Eiffel, París)", search_btn: "Buscar", add_to_day: "Añadir",
    route_mode_walking: "A pie", route_mode_cycling: "Bicicleta", route_mode_driving: "Coche",
    route_total: "Total ruta del día", route_no_transit: "El transporte público no está disponible de forma gratuita; puedes calcular la ruta a pie, en bici o en coche.",
    no_search_results: "Sin resultados. Prueba con otro término.",
  },
  ca: {
    ...
    nav_planner: "Planificador", day_label: "Dia", stop_title_placeholder: "Què fareu aquí?",
    click_map_hint: "Toca el mapa per afegir una parada al dia seleccionat.", no_stops_yet: "Cap punt marcat per aquest dia encara.",
    save: "Desar",
    field_transport_mode: "Mitjà de transport", field_from: "Des de", field_to: "Fins a",
    field_departure_date: "Data de sortida", field_arrival_date: "Data d'arribada",
    field_participants: "Participants", field_files_links: "Arxius/enllaços",
    add_transport_entry: "Afegir transport", no_transport_yet: "Cap transport afegit encara.",
    search_place_placeholder: "Cerca un lloc (ex: Torre Eiffel, París)", search_btn: "Cercar", add_to_day: "Afegir",
    route_mode_walking: "A peu", route_mode_cycling: "Bicicleta", route_mode_driving: "Cotxe",
    route_total: "Total ruta del dia", route_no_transit: "El transport públic no està disponible de forma gratuïta; pots calcular la ruta a peu, en bici o en cotxe.",
    no_search_results: "Sense resultats. Prova un altre terme.",
  },
  en: {
    ...
    nav_planner: "Planner", day_label: "Day", stop_title_placeholder: "What will you do here?",
    click_map_hint: "Tap the map to add a stop to the selected day.", no_stops_yet: "No stops marked for this day yet.",
    save: "Save",
    field_transport_mode: "Mode of transport", field_from: "From", field_to: "To",
    field_departure_date: "Departure date", field_arrival_date: "Arrival date",
    field_participants: "Participants", field_files_links: "Files/links",
    add_transport_entry: "Add transport", no_transport_yet: "No transport added yet.",
    search_place_placeholder: "Search a place (e.g. Eiffel Tower, Paris)", search_btn: "Search", add_to_day: "Add",
    route_mode_walking: "Walking", route_mode_cycling: "Cycling", route_mode_driving: "Driving",
    route_total: "Day route total", route_no_transit: "Public transport isn't available for free; you can calculate the route on foot, by bike, or by car.",
    no_search_results: "No results. Try a different search.",
  },
};
...
function TripFormModal({ lang, title, initialName, initialDestination, initialDays, initialNumPeople, initialModes, submitLabel, savingLabel, onClose, onSubmit }) {
  ...
  const toggleMode = (id) => setModes((ms) => (ms.includes(id) ? ms.filter((x) => x !== id) : [...ms, id]));

  const submit = async (e) => {
    e.preventDefault(); setSaving(true);
    await onSubmit({ name, destination, days, numPeople, modes });
    setSaving(false);
  };
  return (
    <ModalShell title={title} onClose={onClose}>
      <form onSubmit={submit}>
        ...
        <Field label={t(lang, "field_destination")}><input style={inputStyle} required value={destination} onChange={(e) => setDestination(e.target.value)} placeholder={t(lang, "destination_placeholder")} /></Field>
        <Field label={t(lang, "field_days")}><input style={inputStyle} type="number" min={1} required value={days} onChange={(e) => setDays(Number(e.target.value))} /></Field>
        ...
        <Field label={t(lang, "field_transport")}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {TRANSPORT_OPTIONS.map((opt) => {
              const Icon = opt.icon; const active = modes.includes(opt.id);
              return (
                <button type="button" key={opt.id} onClick={() => toggleMode(opt.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, cursor: "pointer", border: `1.5px solid ${active ? COLORS.ocean : COLORS.inkLight + "55"}`, background: active ? COLORS.ocean : "transparent", color: active ? COLORS.paper : COLORS.ink, fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5 }}>
                  <Icon size={15} /> {t(lang, opt.labelKey)} {active && <Check size={13} style={{ marginLeft: "auto" }} />}
                </button>
              );
            })}
          </div>
        </Field>
        <Btn type="submit" disabled={saving || modes.length === 0} style={{ width: "100%", justifyContent: "center", marginTop: 4 }}>{saving ? savingLabel : submitLabel}</Btn>
      </form>
    </ModalShell>
  );
}
...
function CreateTripModal({ lang, onClose, onCreate }) {
  return (
    <TripFormModal lang={lang} title={t(lang, "modal_new_trip")} initialName="" initialDestination="" initialDays={5} initialNumPeople={2} initialModes={["avion"]}
      submitLabel={t(lang, "create_trip")} savingLabel={t(lang, "creating")}
      onClose={onClose} onSubmit={onCreate} />
  );
}

function EditTripModal({ lang, trip, onClose, onSave }) {
  return (
    <TripFormModal lang={lang} title={t(lang, "modal_edit_trip")} initialName={trip.name} initialDestination={trip.destination} initialDays={trip.days} initialNumPeople={trip.num_people} initialModes={trip.transport_modes || [trip.transport_id]}
...
const SECTIONS = [
  ...
  { id: "planificador", labelKey: "nav_planner", icon: MapIcon },
...
function TripDetail({ tripId, lang, displayName, onBack, onToast }) {
  ...
  const load = useCallback(async () => {
    const [{ data: tripRow }, { data: memberRows }, { data: te }, { data: ac }, { data: doc }, { data: ex }, { data: st }] = await Promise.all([
      ...
      supabase.from("planner_stops").select("*").eq("trip_id", tripId).order("created_at"),
    ]);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`trip-${tripId}`)
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "planner_stops", filter: `trip_id=eq.${tripId}` }, load)
      .subscribe();
  ...
  const modes = trip.transport_modes && trip.transport_modes.length ? trip.transport_modes : [trip.transport_id];
  const opt = TRANSPORT_OPTIONS.find((o) => o.id === modes[0]) || TRANSPORT_OPTIONS[0];
  ...
  const saveEdit = async ({ name, destination, days, numPeople, modes: newModes }) => {
    await supabase.from("trips").update({ name, destination, days, num_people: numPeople, transport_id: newModes[0], transport_modes: newModes }).eq("id", tripId);
    setShowEdit(false);
    load();
  };
  ...
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "26px 24px 110px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12, marginBottom: 22 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, color: COLORS.mustard, marginBottom: 4, flexWrap: "wrap" }}>
            {modes.map((mId) => {
              const mOpt = TRANSPORT_OPTIONS.find((o) => o.id === mId) || TRANSPORT_OPTIONS[0];
              return <mOpt.icon key={mId} size={15} />;
            })}
            <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.6 }}>{modes.map((mId) => t(lang, (TRANSPORT_OPTIONS.find((o) => o.id === mId) || TRANSPORT_OPTIONS[0]).labelKey)).join(" · ")}</span>
      ...
      {showEdit && <EditTripModal lang={lang} trip={trip} onClose={() => setShowEdit(false)} onSave={saveEdit} />}
      ...
      {!section ? (
        ...
        <div style={{ background: COLORS.paper, borderRadius: 18, padding: 26 }}>
          <SectionErrorBoundary key={section}>
            {section === "resumen" && <ResumenTab trip={trip} modes={modes} members={members} lang={lang} />}
            {section === "transporte" && <TransporteTab tripId={tripId} modes={modes} displayName={displayName} lang={lang} transportEntries={transportEntries} members={members} onReload={load} />}
            {section === "alojamiento" && <AlojamientoTab tripId={tripId} lang={lang} accommodations={accommodations} onReload={load} />}
            {section === "planificador" && <PlanificadorTab tripId={tripId} lang={lang} displayName={displayName} trip={trip} stops={stops} onReload={load} />}
...
function ResumenTab({ trip, modes, members, lang }) {
  return (
    <div>
      <p style={{ fontFamily: "'Space Grotesk', sans-serif", color: COLORS.ink, lineHeight: 1.7 }}>
        {t(lang, "summary_body", { count: members.length, plural: members.length !== 1 ? (lang === "en" ? "" : "s") : "", name: trip.name, destination: trip.destination, days: trip.days, transport: modes.map((mId) => t(lang, (TRANSPORT_OPTIONS.find((o) => o.id === mId) || TRANSPORT_OPTIONS[0]).labelKey).toLowerCase()).join(", ") })}
      </p>
      <TicketDivider />
...
function TransporteTab({ tripId, modes, displayName, lang, transportEntries, members, onReload }) {
  const blank = { mode: modes[0], origin: "", departureDate: "", destination: "", arrivalDate: "", participants: members };
  ...
  const toggleParticipant = (m) => setForm((f) => ({ ...f, participants: f.participants.includes(m) ? f.participants.filter((x) => x !== m) : [...f.participants, m] }));

  const startEdit = (e) => {
    setEditId(e.id);
    setForm({ mode: e.mode || modes[0], origin: e.origin || "", departureDate: e.departure_date || "", destination: e.destination || "", arrivalDate: e.arrival_date || "", participants: e.participants || members });
    setFile(null); setLink(e.url || "");
  };
  const cancelEdit = () => { setEditId(null); setForm(blank); setFile(null); setLink(""); };
  ...
  const submit = async () => {
    ...
    if (file) { const up = await uploadReceipt(tripId, file); if (up) url = up.url; }
    const payload = {
      mode: form.mode, origin: form.origin.trim(), departure_date: form.departureDate || null,
      destination: form.destination.trim(), arrival_date: form.arrivalDate || null, participants: form.participants, url,
    };
    const { error } = editId
      ? await supabase.from("transport_entries").update(payload).eq("id", editId)
      : await supabase.from("transport_entries").insert({ trip_id: tripId, member: displayName, ...payload });
    setBusy(false);
    if (error) { setSaveError(error.message); return; }
    cancelEdit(); onReload();
  };
  const del = async (id) => { if (!window.confirm(t(lang, "delete_confirm"))) return; await supabase.from("transport_entries").delete().eq("id", id); onReload(); };

  return (
    <div>
      <Field label={t(lang, "field_transport_mode")}>
        <select style={inputStyle} value={form.mode} onChange={(e) => setForm({ ...form, mode: e.target.value })}>
          {modes.map((mId) => {
            const mOpt = TRANSPORT_OPTIONS.find((o) => o.id === mId) || TRANSPORT_OPTIONS[0];
            return <option key={mId} value={mId}>{t(lang, mOpt.labelKey)}</option>;
          })}
        </select>
      </Field>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 130 }}>
          <Field label={t(lang, "field_from")}><input style={inputStyle} value={form.origin} onChange={(e) => setForm({ ...form, origin: e.target.value })} placeholder={t(lang, "field_from")} /></Field>
        </div>
        <div style={{ flex: 1, minWidth: 130 }}>
          <Field label={t(lang, "field_departure_date")}><input style={inputStyle} type="date" value={form.departureDate} onChange={(e) => setForm({ ...form, departureDate: e.target.value })} /></Field>
        </div>
      </div>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 130 }}>
          <Field label={t(lang, "field_to")}><input style={inputStyle} value={form.destination} onChange={(e) => setForm({ ...form, destination: e.target.value })} placeholder={t(lang, "field_to")} /></Field>
        </div>
        <div style={{ flex: 1, minWidth: 130 }}>
          <Field label={t(lang, "field_arrival_date")}><input style={inputStyle} type="date" value={form.arrivalDate} onChange={(e) => setForm({ ...form, arrivalDate: e.target.value })} /></Field>
        </div>
      </div>
      <Field label={t(lang, "field_participants")}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {members.map((m) => (
            <button type="button" key={m} onClick={() => toggleParticipant(m)} style={{ padding: "6px 12px", borderRadius: 999, cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5, border: `1.5px solid ${form.participants.includes(m) ? COLORS.ocean : COLORS.inkLight + "55"}`, background: form.participants.includes(m) ? COLORS.ocean : "transparent", color: form.participants.includes(m) ? COLORS.paper : COLORS.ink }}>{m}</button>
          ))}
        </div>
      </Field>
      <Field label={t(lang, "field_files_links")}>
        <input style={{ ...inputStyle, marginBottom: 8 }} value={link} onChange={(e) => setLink(e.target.value)} placeholder={t(lang, "doc_url_placeholder")} />
        <ReceiptPicker lang={lang} file={file} setFile={setFile} existingUrl={editId ? link : ""} />
      </Field>
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <Btn onClick={submit} disabled={busy}>{editId ? <Check size={15} /> : <Plus size={15} />} {editId ? t(lang, "save_changes") : t(lang, "add_transport_entry")}</Btn>
        {editId && <Btn variant="ghost" onClick={cancelEdit}>{t(lang, "cancel")}</Btn>}
      </div>
      {saveError && <p style={{ color: COLORS.rust, fontSize: 12.5, fontFamily: "'Space Grotesk', sans-serif", marginTop: -12, marginBottom: 16 }}>{saveError}</p>}

      <TicketDivider />
      {transportEntries.length === 0 && <EmptyState text={t(lang, "no_transport_yet")} />}
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {transportEntries.map((e) => {
          const mOpt = TRANSPORT_OPTIONS.find((o) => o.id === e.mode) || TRANSPORT_OPTIONS[0];
          return (
            <div key={e.id} style={{ background: COLORS.paperDim, borderRadius: 10, padding: "12px 14px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <mOpt.icon size={16} color={COLORS.rust} />
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, color: COLORS.ink }}>{e.origin} → {e.destination}</span>
                </div>
                <div style={{ display: "flex", gap: 4 }}>
                  {e.url && <a href={e.url} target="_blank" rel="noreferrer" style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, color: COLORS.ocean, marginRight: 4, alignSelf: "center" }}>{t(lang, "view_receipt")}</a>}
                  {e.member === displayName && <IconBtn icon={Pencil} onClick={() => startEdit(e)} />}
                  {e.member === displayName && <IconBtn icon={Trash2} color={COLORS.rust} onClick={() => del(e.id)} />}
                </div>
              </div>
              <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, color: COLORS.inkLight, marginTop: 4 }}>
                {[e.departure_date, e.arrival_date].filter(Boolean).join(" → ")}
              </div>
              <div style={{ display: "flex", marginTop: 8 }}>
                {(e.participants || []).map((m, i) => (<div key={m} style={{ marginLeft: i === 0 ? 0 : -8 }} title={m}><Avatar name={m} members={members} size={22} /></div>))}
              </div>
            </div>
          );
        })}
...
const ROUTE_MODES = [
  { id: "foot", icon: Footprints, labelKey: "route_mode_walking" },
  { id: "bike", icon: Bike, labelKey: "route_mode_cycling" },
  { id: "car", icon: Car, labelKey: "route_mode_driving" },
];
...
function PlanificadorTab({ tripId, lang, displayName, trip, stops, onReload }) {
  ...
  const [route, setRoute] = useState({ segments: [], distance: 0, duration: 0, error: false });
  ...
  useEffect(() => {
    ...
    fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(dest)}`)
      .then((r) => r.json())
      .then((res) => { if (res && res[0]) setCenter([Number(res[0].lat), Number(res[0].lon)]); })
      .catch(() => {});
  }, [trip?.destination]);

  const dayStops = stops.filter((s) => s.day === activeDay).sort((a, b) => new Date(a.created_at) - new Date(b.created_at));

  const search = async () => {
    if (!query.trim()) return;
    setSearching(true);
    try {
      const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=5&q=${encodeURIComponent(query.trim())}`);
      setResults(await r.json());
    } catch { setResults([]); }
    setSearching(false);
  };

  const addStop = async (result) => {
    await supabase.from("planner_stops").insert({
      trip_id: tripId, day: activeDay, title: result.display_name.split(",").slice(0, 2).join(","),
      lat: Number(result.lat), lng: Number(result.lon), added_by: displayName,
    });
    setResults([]); setQuery(""); onReload();
  };
  const removeStop = async (id) => { if (!window.confirm(t(lang, "delete_confirm"))) return; await supabase.from("planner_stops").delete().eq("id", id); onReload(); };

  useEffect(() => {
    let cancelled = false;
    async function computeRoute() {
      if (dayStops.length < 2) { setRoute({ segments: [], distance: 0, duration: 0, error: false }); return; }
      const segments = [];
      let totalDist = 0, totalDur = 0, hadError = false;
      for (let i = 0; i < dayStops.length - 1; i++) {
        const a = dayStops[i], b = dayStops[i + 1];
        try {
          const r = await fetch(`https://router.project-osrm.org/route/v1/${mode}/${a.lng},${a.lat};${b.lng},${b.lat}?overview=full&geometries=geojson`);
          const data = await r.json();
          const rt = data.routes && data.routes[0];
          if (rt) {
            segments.push(rt.geometry.coordinates.map(([lon, lat]) => [lat, lon]));
            totalDist += rt.distance; totalDur += rt.duration;
          } else hadError = true;
        } catch { hadError = true; }
      }
      if (!cancelled) setRoute({ segments, distance: totalDist, duration: totalDur, error: hadError });
    }
    computeRoute();
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(dayStops.map((s) => s.id)), mode]);

  return (
    <div>
      <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 16, paddingBottom: 4 }}>
        {Array.from({ length: totalDays }, (_, i) => i + 1).map((d) => (
          <button key={d} onClick={() => setActiveDay(d)} style={{ padding: "7px 14px", borderRadius: 999, whiteSpace: "nowrap", border: `1.5px solid ${activeDay === d ? COLORS.ocean : COLORS.inkLight + "55"}`, background: activeDay === d ? COLORS.ocean : "transparent", color: activeDay === d ? COLORS.paper : COLORS.ink, fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, cursor: "pointer" }}>
            {t(lang, "day_label")} {d}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
        <input style={{ ...inputStyle, flex: 1 }} value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && search()} placeholder={t(lang, "search_place_placeholder")} />
        <Btn onClick={search} disabled={searching}><Search size={15} /> {t(lang, "search_btn")}</Btn>
      </div>
      {results.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14, background: COLORS.paperDim, borderRadius: 10, padding: 8 }}>
          {results.map((r, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: "6px 8px" }}>
              <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5, color: COLORS.ink }}>{r.display_name}</span>
              <Btn onClick={() => addStop(r)} style={{ padding: "6px 10px", fontSize: 12, flexShrink: 0 }}><Plus size={13} /> {t(lang, "add_to_day")}</Btn>
            </div>
          ))}
        </div>
      )}
      {searching === false && query === "" && results.length === 0 && null}
      {!searching && results.length === 0 && query.trim() !== "" && <EmptyState text={t(lang, "no_search_results")} />}

      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        {ROUTE_MODES.map((rm) => {
          const RIcon = rm.icon; const active = mode === rm.id;
          return (
            <button key={rm.id} onClick={() => setMode(rm.id)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 999, cursor: "pointer", border: `1.5px solid ${active ? COLORS.ocean : COLORS.inkLight + "55"}`, background: active ? COLORS.ocean : "transparent", color: active ? COLORS.paper : COLORS.ink, fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5 }}>
              <RIcon size={14} /> {t(lang, rm.labelKey)}
            </button>
          );
        })}
      </div>
      <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 11.5, color: COLORS.inkLight, marginBottom: 10 }}>{t(lang, "route_no_transit")}</p>

      <div style={{ borderRadius: 14, overflow: "hidden", marginBottom: 10, border: `1.5px solid ${COLORS.inkLight}33` }}>
        <MapContainer center={center} zoom={12} style={{ height: 340, width: "100%" }}>
          <TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
          <MapAutoFit points={dayStops.map((s) => [s.lat, s.lng])} />
          {dayStops.map((s, i) => (
            <Marker key={s.id} position={[s.lat, s.lng]}><Popup>{i + 1}. {s.title}</Popup></Marker>
          ))}
          {route.segments.map((coords, i) => (
            <Polyline key={i} positions={coords} pathOptions={{ color: "#2E6E8E", weight: 4, opacity: 0.85 }} />
          ))}
        </MapContainer>
      </div>

      {dayStops.length >= 2 && (
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: COLORS.ink, borderRadius: 10, padding: "10px 14px", marginBottom: 16 }}>
          <Navigation size={16} color={COLORS.mustard} />
          <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5, color: COLORS.paper }}>
            {t(lang, "route_total")}: {route.error ? "—" : `${fmtDistance(route.distance)} · ${fmtDuration(route.duration, lang)}`}
          </span>
        </div>
      )}

      {dayStops.length === 0 && <EmptyState text={t(lang, "no_stops_yet")} />}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {dayStops.map((s, i) => (
          <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: COLORS.paperDim, borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, color: COLORS.inkLight }}>{i + 1}</span>
              <MapIcon size={15} color={COLORS.ocean} />
              <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13.5, color: COLORS.ink, fontWeight: 600 }}>{s.title}</span>
            </div>
            <IconBtn icon={Trash2} color={COLORS.rust} onClick={() => removeStop(s.id)} />
          </div>
        ))}
      </div>
    </div>
...
export default function App() {
  ...
  const handleCreateTrip = async (form) => {
    const code = genCode();
    const { data: tripRow, error } = await supabase.from("trips").insert({ code, name: form.name, destination: form.destination, days: form.days, transport_id: form.modes[0], transport_modes: form.modes, num_people: form.numPeople, created_by: session.user.id }).select().single();
